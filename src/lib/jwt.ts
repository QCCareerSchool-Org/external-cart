...
import type { SchoolName } from '@/domain/schoolName';
...
const secrets: Record<SchoolName, Buffer<ArrayBuffer>> = {
...
const getInfo = (school: SchoolName): [issuer: string, audience: string, secret: Buffer<ArrayBuffer>] => {
  let urn: string;
...
export const decodeJwt = async (jwt: string, school: SchoolName): Promise<Result<Record<string, unknown>>> => {
  const [ issuer, audience, secret ] = getInfo(school);

  try {
    const { payload } = await jwtDecrypt<Record<string, unknown>>(jwt, secret, { issuer, audience });
